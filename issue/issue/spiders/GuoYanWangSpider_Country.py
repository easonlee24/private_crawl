...
class GuoYanWangSpider_Country(scrapy.Spider):
    name = "GuoYanWangSpider_Country"
    ...
    def parse(self, response):
        ...
        searchresults = response.xpath(".//li[@class='pub_content']")
        for item in searchresults:
            access_url = item.xpath(".//li/a/@href").extract_first()
            meta['access_url'] = access_url
            meta['pdf_url'] = access_url + "&downloadflag=down"
            keyWords = item.xpath(".//div[@class='mt10']/div[@style]/text()").extract_first()
            # release_date = item.xpath(".//li/span[@class='date de-date']/text()").extract_first()
            # meta['release_date'] = release_date.replace('[ ', '').replace(' ]', '')
            meta['keywords'] = keyWords.replace('，', ';')
            # author_source = item.xpath(".//ul[@class='extra clearfix']/li")
            # author = ""
            # collection = ""
    ...
    def parse_issue(self, response):
        ...
        title = response.xpath(".//div[@id='docSubject']/text()").extract_first()
        release_date = response.xpath(".//div[@id='docDeliveddate']/text()").extract_first()
        abstract = "".join(response.xpath(".//div[@id='docSummary']//text()").extract())
        editor = "".join(response.xpath(".//span[@id='docEditor']//text()").extract())
        author = "".join(response.xpath(".//span[@id='docAuthor']//text()").extract())
        collection = "".join(response.xpath(".//span[@id='docSource']//text()").extract())
