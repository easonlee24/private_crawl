...
class bio_conferenceSpider(scrapy.Spider):
    name = "bio_conference"
    infos = []

    def __init__(self, urlPath=None, *args, **kwargs):
        super(bio_conferenceSpider, self).__init__(*args, **kwargs)
    ...
    def parse_issue(self, response):
        ...
        author_sup = []
        author_affiliation = []
        if len(response.xpath(".//div[@class='summary full']"))>0:
            tableinfo = response.xpath(".//div[@class='summary full']//tr")
            ...
            title ="".join(response.xpath(
                ".//div[@id='contenu']/div[@id='head']/h2[@class='title']/text()").extract()).strip()
            result['title'] = title
            ...
            abstractxpath = response.xpath(".//div[@id='contenu']/div[@id='head']/p[@class='no_abs']")
            abstract = "wrong"
            # preceding-sibling
            if len(abstractxpath) == 0:
                abstract = "".join(response.xpath(".//div[@id='contenu']/div[@id='head']/p[@class='bold']").xpath(
                    "following-sibling::p[1]/text()").extract())
            result['abstract'] = abstract

            authorxpath = response.xpath(".//div[@id='contenu']/div[@id='head']/div[@class='article-authors']")
            if len(authorxpath) > 0:
                ...
                affiliationxpath = response.xpath(".//div[@id='contenu']/div[@id='head']/p[@class='aff']")
